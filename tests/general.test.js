...
describe("[POPULATE]", () => {
  test("Populate single reference", () => {
    ...
    const data = JSON.parse(
      fs.readFileSync("./test-db/products.json", "UTF-8")
    );
  ...
  test("Error handling for non-array references", () => {
    ...
    fs.writeFileSync(
      "./test-db/products.json",
      JSON.stringify({ id: 1, name: "Product 1" }, null, 2)
    );
...
describe("[SELECT PICK]", () => {
  ...
  test("Selecting desired fields", () => {
    const result = db
      .get("users.posts.first.sidePost")
      .selectPick(["title", "author"]);
    const data = JSON.parse(
      fs.readFileSync("./test-db/users/posts/first.json", "UTF-8")
    );
...
describe("[CHECK]", () => {
  ...
  test("Checking if timestamp has passed", () => {
    ...
    db.get("users.posts.first.will_update_at")
      .setTimestamp()
      .advanceTime(10000);
...
describe("[FORMAT TIMESTAMP]", () => {
  ...
  test("Short format", () => {
    const result = db
      .get("users.posts.first.timestamp")
      .formatTimestamp(TimeFormat.SHORT);
    const data = JSON.parse(
      fs.readFileSync("./test-db/users/posts/first.json", "UTF-8")
    );
  ...
  test("Medium format", () => {
    const result = db
      .get("users.posts.first.timestamp")
      .formatTimestamp(TimeFormat.MEDIUM);
    const data = JSON.parse(
      fs.readFileSync("./test-db/users/posts/first.json", "UTF-8")
    );
  ...
  test("Long format", () => {
    const result = db
      .get("users.posts.first.timestamp")
      .formatTimestamp(TimeFormat.LONG);
    const data = JSON.parse(
      fs.readFileSync("./test-db/users/posts/first.json", "UTF-8")
    );
...
describe("[FORMAT RELATIVE TIME]", () => {
  ...
  test("Relative time (hour, past)", () => {
    const result = db
      .get("users.posts.first.pastTimestamp")
      .formatRelativeTime();
  ...
  test("Relative time (hour, future)", () => {
    const result = db
      .get("users.posts.first.futureTimestamp")
      .formatRelativeTime();
    const rtf = new Intl.RelativeTimeFormat("en", { numeric: "auto" });
